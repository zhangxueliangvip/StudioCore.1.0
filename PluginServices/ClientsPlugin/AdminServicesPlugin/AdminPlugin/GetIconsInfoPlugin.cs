...
{
    public class GetIconsInfoPlugin : BasePluginCore, IGetIconsInfoPlugin
    {
        public TBaseResult<IconModels> GetIconsInfo(string safetySecretKey, bool isUsable, string openId)
        {
            this.SafetySecretKey = safetySecretKey;
            this.Usable = isUsable ? BasePluginType.Type.启用 : BasePluginType.Type.卸载;
            var result = new TBaseResult<IconModels>()
            {
                Code = (int)EnumCore.CodeType.失败,
                Message = "系统错误",
                TData = new IconModels()
            };
            ...
            var model = MenuManager.GetInstance.GetIconsModel(openId);
