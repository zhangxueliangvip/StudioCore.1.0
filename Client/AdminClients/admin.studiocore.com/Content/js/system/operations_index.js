layui.config({
    ...
    var pageTable = table.render({
        elem: '#pageData'
        , url: '/Menu/GetPageDataOperations'
        , cols: [[
            { type: 'checkbox', fixed: 'left' }
            , { field: 'displayID', width: 80, title: 'ID', sort: true, fixed: 'left' }
            , { field: 'displayOperationTitle', title: '名称' }
    ...
    var active = {
        reload: function () {
            table.reload('QuerySearch', {
                ...
                , where: {
                    OperationTitle: $('#OperationTitle').val(),
    ...
    $("#AddData").click(function () {
        layer.open({
            ...
            maxmin: true,
            area: ['560px', '360px'],
            content: '/Menu/AddOperationsDataBox'
        });
...
function Updated(openId) {
    layer.open({
        ...
        maxmin: true,
        area: ['560px', '360px'],
        content: '/Menu/AddOperationsDataBox?openId=' + openId
    });
}
function Deleted(openId) {
    layer.confirm('确定要删除？', {
        ...
        var url = "/Menu/DeleteDataOperations";
        var data = {};
        data.openId = openId;
